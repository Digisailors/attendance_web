...
export const authHelpers = {
  ...
  getCurrentUserWithEmployee: async () => {
    ...
    const { data: employee, error: employeeError } = await employeeQueries.getEmployeeByEmail(user.email!)
  ...
  canAccessResource: async (resourceType: 'employee' | 'team' | 'department', resourceId?: string) => {
    const { employee } = await authHelpers.getCurrentUserWithEmployee()
