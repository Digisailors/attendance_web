...
export const employeeQueries = {
  ...
  // Create new employee
  createEmployee: async (employeeData: Database['public']['Tables']['employees']['Insert']) => {
    const { data, error } = await supabase
      .from('employees')
      .insert(employeeData)
      .select()
  ...
  // Update employee
  updateEmployee: async (id: string, updates: Database['public']['Tables']['employees']['Update']) => {
    const { data, error } = await supabase
      .from('employees')
      .update(updates)
      .eq('id', id)
...
export const attendanceQueries = {
  ...
  getEmployeeMonthlyAttendance: async (employeeId: string, year: number, month: number) => {
  ...
  // Check in
  checkIn: async (employeeId: string, workType: 'Office' | 'WFH' | 'Hybrid') => {
    const today = new Date().toISOString().split('T')[0]
...
export const workSubmissionQueries = {
  ...
  createWorkSubmission: async (submissionData: Database['public']['Tables']['work_submissions']['Insert']) => {
    const { data, error } = await supabase
  ...
  // Reject submission
  rejectSubmission: async (submissionId: string, comments: string, rejectedBy: 'team_lead' | 'manager') => {
    const updates = {
...
export const leaveRequestQueries = {
  ...
  createLeaveRequest: async (leaveData: Database['public']['Tables']['leave_requests']['Insert']) => {
    const { data, error } = await supabase
      .from('leave_requests')
      .insert(leaveData)
      .select()
  ...
  getPendingLeaveRequests: async (approverId: string) => {
    const { data, error } = await supabase
      ...
      `)
      .or(`employees.team_lead_id.eq.${approverId},employees.manager_id.eq.${approverId}`)
      .eq('status', 'Pending')
      .order('created_at', { ascending: false })
...
export const dashboardQueries = {
  ...
  getEmployeeMonthlyStats: async (employeeId: string, year: number, month: number) => {
    const startDate = `${year}-${month.toString().padStart(2, '0')}-01`
    const endDate = `${year}-${month.toString().padStart(2, '0')}-31`
  ...
  getTeamPerformance: async (teamLeadId: string, month?: number, year?: number) => {
