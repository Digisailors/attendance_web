...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("GET leave requests params:", {
      employeeId,
      teamLeadId,
      managerId,
      status,
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    console.log("PATCH leave request:", {
      requestId,
      action,
      userType,
      teamLeadId,
    ...
    if (!requestId || !action || !userType) {
      return NextResponse.json(
        { error: "Missing required fields: requestId, action, userType" },
        { status: 400 }
      );
    }
    ...
    if (userType === "team-lead") {
      if (!teamLeadId || leaveRequest.team_lead_id !== teamLeadId) {
        return NextResponse.json(
          { error: "Unauthorized: Team Lead ID mismatch or missing" },
          { status: 403 }
        );
      }
      ...
      // Replace the manager authorization check in your PATCH method with this:
    } else if (userType === "manager") {
      // Convert both values to strings for comparison
      const dbManagerId = leaveRequest?.manager_id
        ?.toString()
        .trim()
        .toLowerCase();
      const requestManagerId = managerId?.toString().trim().toLowerCase();

      if (
        !leaveRequest?.manager_id ||
        !managerId ||
        dbManagerId !== requestManagerId
      ) {
        console.error("Manager ID mismatch:", {
          dbManagerId: leaveRequest?.manager_id,
          requestManagerId: managerId,
          dbManagerIdProcessed: dbManagerId,
          requestManagerIdProcessed: requestManagerId,
        });

        return NextResponse.json(
          { error: "Unauthorized: Manager ID mismatch or missing" },
          { status: 403 }
        );
      }
