...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!requestId || !action || !teamLeadId || !userType) {
      return NextResponse.json(
        { error: "Missing required fields: requestId, action, teamLeadId, userType" },
        { status: 400 },
      )
    }
  ...
  return NextResponse.json(
    ...
    return NextResponse.json({
      message: `Leave request ${action === "approve" ? "approved and sent to manager" : "rejected"} successfully`,
      data: updatedRequest,
    })
  } catch (error) {
