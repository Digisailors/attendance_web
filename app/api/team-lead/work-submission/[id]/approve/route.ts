...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Update the submission status to "Pending Final Approval" for manager review
    const { data: updatedSubmission, error: updateError } = await supabase
      .from("work_submissions")
      .update({
        status: "Pending Final Approval", // Changed from "Approved by Team Lead"
      ...
      if (employeeData && !employeeError) {
        const { error: notificationError } = await supabase
          ...
          .insert({
            ...
            title: "Work Submission Approved by Team Lead",
            message: "Your work submission has been approved by your team lead and is now pending final approval from the manager",
            type: "work_approval",
    ...
    return NextResponse.json({
      success: true,
      message: "Work submission approved and sent to manager for final approval",
    })
  } catch (error) {
