...
export default function TeamLeadProfile() {
  ...
  useEffect(() => {
    const initializeUserData = async () => {
      ...
      if (empId) {
        ...
        setError(
          `No team lead profile found for email: ${user.email}. Please contact your administrator.`
        );
        setLoading(false);
        return;
      }
  ...
  // FIXED TEAM LEAD DATA FETCHING - Shows Missed Days instead of Permissions (same as employee)
  const fetchEmployeeData = useCallback(async () => {
    try {
      ...
      if (empError || !employeeRecord) {
        ...
        throw new Error(
          `Team lead profile not found for email: ${loggedInUser.email}`
        );
      }
      ...
      console.log(
        "Using team lead employee ID for work log:",
        actualEmployeeId
      );
      ...
      // ✅ FIXED: Calculate counts correctly (same as employee)
