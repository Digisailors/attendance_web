  ...
  const cleanupOldStates = (employeeId: string) => {
    try {
      ...
      for (let i = 1; i <= 30; i++) {
        ...
        const key = `employee_daily_state_${employeeId}_${pastDate.toISOString().split('T')[0]}`;
...
const handleCheckIn = async () => {
  if (isCheckedIn) {
    console.log("Already checked in â€” skipping API call")
    return
  }
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className={`text-xs mt-1 font-medium ${
                    calculateOvertime() > 0 ? 'text-amber-600' : 'text-gray-500'
                  }`}>
                    {calculateOvertime() > 0 ? `+${calculateOvertime().toFixed(1)} extra hours` : 'Standard hours'}
                  </p>
                </div>
              </div>
            </div>
