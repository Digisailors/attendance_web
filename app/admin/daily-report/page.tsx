...
const transformBackendData = (
  ...
  if (
    !backendResponse?.employees ||
    !Array.isArray(backendResponse.employees)
  ) {
...
const normalizeSummary = (
  summaryData: any,
  transformedEmployees: DailyAttendanceEmployee[],
  selectedDate: string
) => {
  // Calculate from transformed data if backend doesn't provide summary
  const presentCount = transformedEmployees.filter(
    (emp) => emp.attendanceStatus === "Present"
  ).length;
  const absentCount = transformedEmployees.filter(
    (emp) => emp.attendanceStatus === "Absent"
  ).length;
  const lateCount = transformedEmployees.filter(
    (emp) => emp.attendanceStatus === "Late"
  ).length;
...
export default function ReportsPage() {
  ...
  const fetchDailyAttendance = useCallback(
    async (
      ...
      try {
        ...
        if (modeFilter && modeFilter !== "All Modes")
          params.append("workMode", modeFilter);
        ...
        const transformedEmployees = transformBackendData(
          backendData,
          selectedDate
        );
  ...
  const debouncedSearch = useMemo(
    () =>
      debounce(
        (searchValue: string, statusFilter: string, modeFilter: string) => {
  ...
  const exportToExcel = async (
    ...
    try {
      ...
      const excelData = [
        [
          "Daily Attendance Report - " +
            exportType.charAt(0).toUpperCase() +
            exportType.slice(1),
        ],
        [
          "Date: " +
  ...
  const handleSummaryCardClick = (
    filterType: "all" | "present" | "absent" | "late"
  ) => {
    ...
    return attendanceData.map((employee) => (
      <TableRow key={employee.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-center">
          <Badge
            className={getAttendanceStatusBadge(employee.attendanceStatus)}
          >
                ...
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg mb-6">
                  ...
                  <div className="flex space-x-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => fetchDailyAttendance(currentPage)}
                    >
                      Retry
                    </Button>
                    <Button
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          {selectedStatus === "All Status" &&
                            !showMultiSelect && (
                              <>
                                <DropdownMenuItem
                                  onClick={() => exportToPDF("present")}
                                >
                                  <UserCheck className="mr-2 h-4 w-4" />
                                  Present Only - PDF
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => exportToExcel("present")}
                                >
                                  <UserCheck className="mr-2 h-4 w-4" />
                                  Present Only - Excel
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => exportToPDF("absent")}
                                >
                                  <UserX className="mr-2 h-4 w-4" />
                                  Absent Only - PDF
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => exportToExcel("absent")}
                                >
                                  <UserX className="mr-2 h-4 w-4" />
                                  Absent Only - Excel
                                </DropdownMenuItem>
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm font-medium">
                            {selectedEmployeeIds.size ===
                              attendanceData.length && attendanceData.length > 0
                              ? "Deselect All"
                              : "Select All"}
                          </span>
                        </div>
                        <span className="text-sm text-gray-600">
                          {selectedEmployeeIds.size} of {attendanceData.length}{" "}
                          employees selected
                        </span>
                      </div>
