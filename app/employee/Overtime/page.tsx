...
const [currentOTId, setCurrentOTId] = useState<string | null>(null);
  ...
  // Add this new function near your other helper functions (around line 90-100)
  const fetchActiveOTSession = async (empUUID: string, date: string) => {
    try {
      console.log("🔍 Checking for active OT session in database...");
      const response = await fetch(
        `/api/employees/Overtime?employee_id=${empUUID}&date=${date}`
      );

      if (!response.ok) {
        console.error("Failed to fetch active OT session");
        return null;
      }

      const data = await response.json();

      // Check if there's an active OT session (end_time matches start_time means still active)
      if (Array.isArray(data) && data.length > 0) {
        const activeOT = data.find(
          (ot) =>
            ot.ot_date === date &&
            ot.is_active === true &&
            ot.start_time === ot.end_time // This indicates OT is still in progress
        );

        if (activeOT) {
          console.log("✅ Found active OT session:", activeOT);
          return activeOT;
        }
      }

      console.log("No active OT session found");
      return null;
    } catch (error) {
      console.error("Error fetching active OT session:", error);
      return null;
    }
  };
  ...
  // Load active OT session from database when employee data is available
  useEffect(() => {
    const loadActiveOTFromDatabase = async () => {
      if (employeeData?.id && !isOTStarted) {
        const today = getTodayDate();
        const activeOT = await fetchActiveOTSession(employeeData.id, today);

        if (activeOT) {
          console.log("📥 Restoring OT session from database");

          // Restore OT state from database
          setIsOTStarted(true);

          // Parse start_time (it's stored as "HH:MM:SS")
          const [hours, minutes, seconds] = activeOT.start_time.split(":");
          ...
          setOTStartTime(startDateTime);

          // Check if work was already submitted (reason is not placeholder)
          const isWorkAlreadySubmitted =
            activeOT.reason &&
            activeOT.reason !== "OT in progress - work details pending";

          if (isWorkAlreadySubmitted) {
            setIsWorkSubmitted(true);

            // Parse work type and description from reason
            const [workType, ...descParts] = activeOT.reason.split(": ");
            setWorkType(workType || "");
            setWorkDescription(descParts.join(": ") || "");

            console.log("✅ Work was already submitted, restored work details");

            // Note: Images cannot be restored as File objects, user needs to re-upload
            if (!activeOT.image1 || !activeOT.image2) {
              console.warn("⚠️ Images missing - user needs to re-upload");
            }
          }

          console.log("✅ OT session restored successfully");
        }
      }
    };

    loadActiveOTFromDatabase();
  }, [employeeData?.id, isOTStarted]);
  ...
  const checkTodayCheckInStatus = async (empId: string) => {
    try {
      ...
      const today = getTodayDate();
      const response = await fetch(
        `/api/employees/${empId}/worklog?date=${today}`
      );
  ...
  useEffect(() => {
    if (employeeData?.employee_id || user?.email) {
      ...
      if (savedState) {
        ...
        setOTStartTime(
          savedState.otStartTime ? new Date(savedState.otStartTime) : null
        );
        setOTEndTime(
          savedState.otEndTime ? new Date(savedState.otEndTime) : null
        );
  ...
  const handleOTStart = async () => {
    // Check if user has completed their regular day (check-in AND check-out)
    if (!hasCheckedInAndOut) {
      alert(
        "You must complete your regular work day (check-in and check-out) before starting OT. Please go to the main dashboard to check out first."
      );
      return;
    }
    ...
    try {
      ...
      console.log("🚀 Starting OT with data:", {
        employee_id: employeeUUID,
        ot_date: getTodayDate(),
        start_time: startMoment.toISOString(),
      });

      // ✅ Step 1: Call API to save OT start immediately (JSON only - no images)
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to start OT");
      }
const result = await response.json();
console.log("✅ OT started successfully:", result);
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > remainingSlots) {
      alert(
        `You can only upload ${remainingSlots} more image(s). Currently uploaded: ${uploadedImages.length}/2`
      );
      return;
    }
  ...
  const handleOTEnd = async () => {
    ...
    if (uploadedImages.length !== 2) {
      alert(
        "Please upload exactly 2 images before ending OT. Current images: " +
          uploadedImages.length
      );
      return;
    }
    ...
    try {
      ...
      // Get OT ID from state or find it from database
      let otId = currentOTId;

      if (!otId) {
        // Try to get from database if not in state
        const today = getTodayDate();
        const activeOT = await fetchActiveOTSession(employeeUUID, today);
        otId = activeOT?.id;
      }

      if (!otId) {
        alert(
          "Could not find active OT session. Please refresh and try again."
        );
        return;
      }

      console.log("Using OT ID:", otId);
      ...
      // Send to backend (using PUT for work submission + PATCH for end time)
      const putResponse = await fetch("/api/employees/Overtime", {
        method: "PUT",
        body: formData,
      });

      if (!putResponse.ok) {
        throw new Error("Failed to submit work details");
      }

      // Now end the OT
      const patchResponse = await fetch("/api/employees/Overtime", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!patchResponse.ok) {
        throw new Error("Failed to end OT");
      }
  ...
  const getCurrentOTHours = () => {
    if (otStartTime && !otEndTime) {
      const now = new Date();
      const totalHours =
        (now.getTime() - otStartTime.getTime()) / (1000 * 60 * 60);
      return Math.max(0, parseFloat(totalHours.toFixed(2)));
    }

    if (otStartTime && otEndTime) {
      const totalHours =
        (otEndTime.getTime() - otStartTime.getTime()) / (1000 * 60 * 60);
      return Math.max(0, parseFloat(totalHours.toFixed(2)));
    }
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center mt-2">
                      ...
                      >
                        {isOTEnded
                          ? "OT session completed"
                          : isOTStarted
                          ? "Currently working OT"
                          : "Ready to start OT"}
                      </p>
                    </div>
                  </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-red-800 font-medium">
                    Regular Work Day Completion Required
                  </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-green-800 font-medium">
                    Regular Work Day Completed
                  </div>
                  <div className="text-green-700 text-sm">
                    Checked in at {todayCheckInData.check_in} and checked out at{" "}
                    {todayCheckInData.check_out}. You can start OT now.
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-amber-700 text-sm">
                      Checked in at {todayCheckInData.check_in}. Please check
                      out first to complete your regular work day before
                      starting OT.
                    </div>
                  </div>
                </div>
              ...
              <div className="mx-6 mb-4 bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-red-800 text-sm font-medium">
                    ⚠️ Images missing! Please re-upload your 2 work evidence
                    images to end OT.
                  </p>
                </div>
              </div>
              ...
              <div className="mx-6 mb-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-amber-800 text-sm font-medium">
                    Upload {2 - uploadedImages.length} more image(s) to complete
                    your OT submission.
                  </p>
                </div>
              </div>
                        ...
                        <div>
                          ...
                          {!hasCheckedInAndOut && (
                            <p className="text-sm text-red-600 mt-2">
                              You must complete your regular work day (check-in
                              and check-out) before starting OT.
                            </p>
                          )}
                        </div>
                      ) : (
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              Current OT Hours: {getCurrentOTHours().toFixed(2)}{" "}
                              hours
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          {uploadedImages.length < 2 && (
                            <p className="text-sm text-red-600">
                              Please upload exactly 2 images to submit your
                              work.
                            </p>
                          )}
                        </div>
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                              ...
                              <div className="flex items-center space-x-2 text-green-600">
                                ...
                                <span className="text-sm font-medium">
                                  OT work submitted successfully
                                </span>
                              </div>
                            </div>
                          </div>
                            ...
                            uploadedImages.length < 2 && (
                              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                                <div className="text-amber-800 mb-3">
                                  <strong>
                                    Upload {2 - uploadedImages.length} more
                                    image(s)
                                  </strong>{" "}
                                ...
                                <div className="flex items-center space-x-4 mb-4">
                                  ...
                                  <div className="text-sm text-amber-600">
                                    <p>
                                      • Need {2 - uploadedImages.length} more
                                      image(s)
                                    </p>
                                    <p>• Max file size: 5MB each</p>
                          ...
                          <div className="border-t pt-6">
                            ...
                            <div className="mb-4">
                              <div className="text-sm text-gray-700">
                                <strong>Current OT Hours:</strong>{" "}
                                {getCurrentOTHours().toFixed(2)} hours
                              </div>
                            </div>
                            <button
                            ...
                            >
                              ...
                              <span>
                                {isSubmittingWorkLog
                                  ? "Submitting to Team lead..."
                                  : "End OT"}
                              </span>
                            </button>
                            {uploadedImages.length !== 2 && (
                              <p className="text-sm text-red-600 mt-2">
                                Please upload exactly 2 images before ending OT.
                              </p>
                            )}
                          </div>
                        </div>
