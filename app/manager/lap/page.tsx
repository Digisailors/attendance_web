...
export default function ManagerLeavePermissionRequests() {
  ...
  const needsManagerApproval = (
    ...
    console.log(
      `🔍 Checking status for request ${request.id}: "${request.status}" (normalized: "${status}")`
    );
  ...
  // Function to calculate permission duration
  const calculatePermissionDuration = (
    startTime: string,
    endTime: string
  ): string => {
    try {
  ...
  const fetchAllTeamLeadNames = async (
    requests: (LeaveRequest | PermissionRequest)[]
  ) => {
    const uniqueTeamLeadIds = Array.from(
      new Set(
        requests
      ...
      try {
        ...
        for (const endpoint of endpoints) {
          try {
            ...
            console.warn(
              `Failed to fetch from ${endpoint} for ${teamLeadId}:`,
              error
            );
            continue;
          }
    ...
    if (request.team_lead_id) {
      ...
      if (cached) {
        ...
        if (cached.error) {
          ...
          if (
            request.employee?.team_lead &&
            request.employee.team_lead !== "No Team Lead Assigned"
          ) {
  ...
  const getAppliedDate = (
    ...
    if (
      ...
      request.applied_date !== "Invalid Date" &&
      new Date(request.applied_date).toString() !== "Invalid Date"
    ) {
      return request.applied_date;
    }
    return (
      request.created_at || request.applied_date || new Date().toISOString()
    );
  };
  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const response = await fetch(
        `/api/team-lead/leave-requests?${params.toString()}`
      );
      ...
      if (response.ok) {
        ...
        requests.forEach((req: LeaveRequest, index: number) => {
          console.log(
            `📄 Request ${index + 1} (ID: ${req.id}): Status = "${req.status}"`
          );
        });
        ...
        const pending = requests.filter((r: LeaveRequest) =>
  ...
  // Fetch Permission Requests - Modified to exclude pending status
  const fetchPermissionRequests = async () => {
    try {
      ...
      // Add excludePending parameter to fetch all statuses except pending

      // Only add status filter if it's not "All" and not a pending status
      if (
        statusFilter !== "All" &&
        !statusFilter.toLowerCase().includes("pending")
      ) {
        params.append("status", statusFilter);
      }

      const response = await fetch(
        `/api/team-lead/permission-requests?${params.toString()}`
      );
      ...
      if (response.ok) {
        ...
        // Calculate stats with flexible status matching
        const total = requests.length;
        const pending = requests.filter((r: PermissionRequest) =>
          needsManagerApproval(r)
        ).length;
      ...
      setError(
        `Error fetching permission requests: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  const handleApprovePermission = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/manager/final-approvals/permission-requests`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-purple-700 flex items-center space-x-4">
                      <span>
                        Manager • ID: {managerData?.employee_id || "N/A"}
                      </span>
              ...
              <TabsContent value="leave" className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Review and provide final approval for leave requests
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                                  ...
                                  <div>
                                    ...
                                    <div className="text-sm text-gray-500 flex items-center space-x-4 mt-1">
                                      <div className="flex items-center space-x-1">
                                        <Users className="h-3 w-3" />
                                        <span>
                                          Team Lead:{getTeamLeadName(request)}
                                ...
                                {needsManagerApproval(request) && (
                                  <div className="space-y-4 border-t pt-4">
                                    ...
                                    <div className="flex space-x-2">
                                      <Button
                                        onClick={(e) =>
                                          handleApproveLeave(request.id, e)
                                        }
                                        disabled={
              ...
              <TabsContent value="permission" className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Review and provide final approval for permission requests
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
